...
# ADL-model has following assumptions:
# 1.
# 2.

# time sereis
delta_price.ts = ts(data$delta_price, start=c(2014, 10), end=(2023, 03))
...
#modelsummary(model1, stars=c('***' = 0.01, '**' = 0.05, '*' = 0.1), gof_omit = "^(?!R2|Num)")

plot(data$vola, data$, main="Plot", xlab="Nachfrage", ylab="Vola")
